...
def load_json(path, timeout_sec=5):
    """Load JSON from file with timeout protection"""
    import signal

    def timeout_handler(signum, frame):
        raise TimeoutError(f"File operation timeout after {timeout_sec}s")

    try:
        # Set timeout for file operation
        old_handler = signal.signal(signal.SIGALRM, timeout_handler)
        signal.alarm(int(timeout_sec) + 1)  # +1 to account for processing time
        ...
        logger.warning(f"Timeout loading {path}", str(e))
        return None
...
def cache_find_program(channel=None, title=None, start_ts=None, prefer_past=False):
    ...
    for ev in candidates:
        ...
        if 'FOXNEWS' in ev_channel or 'FOX NEWS' in ev_channel:
            ...
            score += 3  # Preference for major networks
    ...
    if debug_scores:
        ...
        for i, entry in enumerate(sorted_scores):
            logger.debug(f"  Match {i+1}: {entry['channel']} - {entry['title']}", f"score={entry['score']}")
...
def notify():
    ...
    # because cache lookup by start time is more accurate than API search by title only
